...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, batch_size=None, n_ctx=4, ctx_init=None, ctx_position='end', learned_cls=False, num_p=1, featmemo=None, num_select_p=1):
        super().__init__()
...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False, num_p=1, featmemo=None, num_select_p=1):
        ...
        # prompt tuning
        self.num_p = num_p
        self.prompt_learner = PromptLearner(clip, classnames, batch_size, n_ctx, ctx_init, ctx_position, learned_cls, num_p, featmemo, num_select_p)
...
def get_coop(clip_arch, test_set, device, n_ctx, ctx_init, learned_cls=False, num_p=1, featmemo=None, num_select_p=1):
    ...
    model = ClipTestTimeTuning(device, classnames, None, arch=clip_arch,
                            n_ctx=n_ctx, ctx_init=ctx_init, learned_cls=learned_cls, num_p=num_p, featmemo=featmemo, num_select_p=num_select_p)
