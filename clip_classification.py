...
def main_worker(gpu, args):
    ...
    logname = 'logs/' + "{}{}_ctxinit_{}.txt".format(args.log_date, args.lr, args.ctx_init[:10])
    ...
    for id in results.keys():
        log_string(log_file, "{}".format(id), end="	")
    log_string(log_file, "\n")
    for id in results.keys():
        log_string(log_file, "{:.2f}".format(results[id][0]), end="	")
    log_string(log_file, "\n")
